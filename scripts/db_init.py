...
def build_vocab_from_v1(con):
    ...
    for r in rows:
        ...
        modality = "acoustic"
        cur = con.execute("""
            INSERT OR IGNORE INTO signal_terms (signal_id, canonical_label, acoustic_descriptor, modality, definition)
            VALUES (?, ?, ?, ?, ?)
    ...
    for r in rows:
        ...
        if not val: continue
        slug = slugify(val)
        cur = con.execute("INSERT OR IGNORE INTO context_terms (context_id, canonical_label) VALUES (?, ?)",
                         (slug, val))
        if cur.rowcount:
            counts["context"] += 1
    ...
    for r in rows:
        ...
        if any(w in v for w in ["mate attraction","male competition","territory","predator avoid"]):
            ...
            level = "biological"
        cur = con.execute("""
            INSERT OR IGNORE INTO function_terms (function_id, canonical_label, level)
            VALUES (?, ?, ?)
        """, (slug, val, level))
        if cur.rowcount:
            counts["function"] += 1
